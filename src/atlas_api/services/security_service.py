    ...
    def normalize_symbol(self, symbol: str) -> str:
        ...
        if not (SYMBOL_MIN_LENGTH <= len(normalized) <= SYMBOL_MAX_LENGTH):
            raise InvalidSymbolFormatError(
                f"Symbol '{symbol}' must be between {SYMBOL_MIN_LENGTH} and {SYMBOL_MAX_LENGTH} characters long."
            )
        if not SYMBOL_PATTERN.match(normalized):
            raise InvalidSymbolFormatError(
                f"Symbol '{symbol}' contains invalid characters. Only letters, dots, and hyphens are allowed."
            )
        return normalized
    ...
    def get_security(self, symbol: str) -> SecurityRead:
        ...
        if not security:
            raise SecurityNotFoundError(
                f"Security with symbol '{normalized}' not found."
            )
        return self._to_read(security)
