...
class PortfolioAnalyticsService:
    ...
    async def get_portfolio_analytics(self, 
                                portfolio_id: UUID, 
                                user_id: UUID) -> PortfolioAnalyticsRead:
        if not self.verify_portfolio_ownership(portfolio_id, user_id):
            raise PortfolioNotFoundError("User does not have ownership of the portfolio.")
        
        positions = self.position_repository.get_all_positions(portfolio_id)
        if not positions:
            return self.analysis_calculations.calculate_portfolio_analytics(portfolio_id, [])
        ...
        return self.analysis_calculations.calculate_portfolio_analytics(portfolio_id, position_analytics)
