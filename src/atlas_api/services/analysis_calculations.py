...
class AnalysisCalculations:
    ...
    def calculate_unrealized_gain_loss(
        current_price: Decimal, average_cost: Decimal, shares: Decimal
    ) -> Decimal:
    ...
    @staticmethod
    def calculate_unrealized_gain_loss_percent(
        unrealized_gain_loss: Decimal, cost_basis: Decimal
    ) -> Decimal | None:
        if cost_basis == 0:
            return None
        return AnalysisCalculations.round_money(
            (unrealized_gain_loss / cost_basis) * 100
        )

    @staticmethod
    def calculate_allocation_percent(
        position_market_value: Decimal, total_market_value: Decimal
    ) -> Decimal:
        if total_market_value == 0:
            return Decimal("0.00")
        return AnalysisCalculations.round_money(
            (position_market_value / total_market_value) * 100
        )
    ...
    def calculate_portfolio_analytics(
        ...
        total_unrealized_gain_loss = self.round_money(
            total_market_value - total_cost_basis
        )
        total_unrealized_gain_loss_percent = (
            self.calculate_unrealized_gain_loss_percent(
